...
#
#  Repo   : https://github.com/yourrepo/cloudflare-ufw-sync
...
MIN_V4_EXPECTED=5        # Abort if Cloudflare returns fewer than this many IPv4 CIDRs
MIN_V6_EXPECTED=3        # Abort if Cloudflare returns fewer than this many IPv6 CIDRs
...
# =============================================================================
#  Atomic replace — only swap in new files after successful validation
# =============================================================================
...
#  State file records exactly which CIDRs were added last run — no awk parsing
#  of UFW output, no fragile grep on rule numbers.
# =============================================================================
...
if [ -f "$STATEFILE" ]; then
  while IFS= read -r old_ip; do
    ...
    if ufw delete allow proto tcp from "$old_ip" to any port 80,443 2>/dev/null; then
      removed=$((removed + 1))
    else
...
while IFS= read -r cfip; do
  ...
  ufw allow proto tcp from "$cfip" to any port 80,443 comment "$COMMENT"
  echo "$cfip" >> "$STATEFILE"
...
if ufw reload; then
  ...
  log "ERROR: UFW reload failed — rules may be in an inconsistent state"
  log "       Run 'ufw reload' manually and inspect 'ufw status numbered'"
  exit 1
fi
